...
ADDITIONAL_CATEGORIES = ["wireless earbuds", "coffee machines"]
...
async def main():
    ...
    try:
        ...
        all_products = []
        print("No existing products found, starting fresh")

    scraped_urls = {p.get("product_url") for p in all_products if p.get("product_url")}
    product_id = max([p.get("id", 0) for p in all_products], default=0) + 1

    remaining = TARGET_TOTAL - len(all_products)
    if remaining <= 0:
        print(f"Already have {len(all_products)} products, no need to add more")
        return

    print(f"Need to add {remaining} more products")
    ...
    async with async_playwright() as p:
        ...
        for category in ADDITIONAL_CATEGORIES:
            ...
            per_cat = (
                remaining
                // (
                    len(ADDITIONAL_CATEGORIES)
                    - ADDITIONAL_CATEGORIES.index(category)
                    + 1
                )
                + 10
            )
            print(f"\nScraping additional category: {category} (target: {per_cat})")
    ...
    print(f"\nAdded products. Total products: {len(all_products)}")
